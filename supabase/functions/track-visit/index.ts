...
const BOT=/bot|crawl|spider|slurp|google|baidu|yandex|duckduck|semrush|ahrefs|petalbot|bytespider/i;
...
serve(async(req)=>{
  ...
  try{
    ...
    // Log IP to player profile for logged-in users
    ...
    if(ex){
      ...
      if(userId&&!ex.user_id){u.user_id=userId;u.user_name=userName||null;}
      if(clientIp&&!isPriv(clientIp))u.client_ip=clientIp;
      await sb.from("page_visits").update(u).eq("id",ex.id);
    }else{
      const{error}=await sb.from("page_visits").insert({session_id:sessionId,page:page||null,user_id:userId||null,user_name:userName||null,referrer:referrer||null,user_agent:userAgent||null,country,city,lat,lon,visit_count:1,last_seen_at:now,client_ip:clientIp&&!isPriv(clientIp)?clientIp:null});
