...
function HomePage({ data, setPage, onProductClick }) {
  ...
  return (
    <div>
      ...
      {/* ── PHOTO GALLERY STRIP ── */}
      {(() => {
        const allImgs = (data.albums || []).flatMap(a =>
          (a.images || []).map(url => typeof url === "string" ? { url } : url)
        ).filter(i => i.url);
        if (allImgs.length === 0) return null;
        const msPerHour = 3600000;
        const hourSlot = Math.floor(Date.now() / msPerHour) % Math.max(1, allImgs.length);
        const shuffled = [...allImgs.slice(hourSlot), ...allImgs.slice(0, hourSlot)].slice(0, 8);
        return (
          <div style={{ background:"#040604", borderTop:"1px solid #2a4018", borderBottom:"1px solid #2a4018", overflow:"hidden" }}>
            <div style={{ padding:"16px 24px 10px", display:"flex", alignItems:"center", gap:16, maxWidth:1200, margin:"0 auto" }}>
              <div style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:8, color:"#5a6e42", letterSpacing:".28em", textTransform:"uppercase" }}>◈ FIELD FOOTAGE</div>
              <div style={{ flex:1, height:1, background:"linear-gradient(90deg,#2a4018,transparent)" }}/>
              <button onClick={() => setPage("gallery")}
                style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:9, letterSpacing:".15em", color:"#c8ff00", background:"none", border:"1px solid rgba(200,255,0,.25)", padding:"4px 14px", cursor:"pointer", textTransform:"uppercase" }}>
                FULL GALLERY →
              </button>
            </div>
            <div style={{ display:"flex", gap:3, paddingBottom:3, overflowX:"auto", WebkitOverflowScrolling:"touch", scrollbarWidth:"none" }}>
              {shuffled.map((img, i) => (
                <div key={i} onClick={() => setPage("gallery")}
                  style={{ flexShrink:0, width: isMobile ? 160 : 220, height: isMobile ? 110 : 150, overflow:"hidden", cursor:"pointer", position:"relative", background:"#111a0a" }}>
                  <img src={img.url} alt="" style={{ width:"100%", height:"100%", objectFit:"cover", opacity:.7 }}/>
                  <div style={{ position:"absolute", inset:0, background:"linear-gradient(180deg,transparent 60%,rgba(4,6,4,.7))", pointerEvents:"none" }}/>
                </div>
              ))}
            </div>
          </div>
      ...
      {(() => {
        ...
        return (
          <div style={{ maxWidth:1200, margin:"0 auto", padding: isMobile ? "28px 16px" : "36px 24px 8px" }}>
            <div style={{ display:"flex", alignItems:"center", gap:16, marginBottom:18 }}>
              <div>
                <div style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:8, color:"#5a6e42", letterSpacing:".22em", textTransform:"uppercase", marginBottom:4 }}>INTEL FEED</div>
                <div style={{ fontFamily:"'Oswald','Barlow Condensed',sans-serif", fontSize:22, fontWeight:700, letterSpacing:".18em", textTransform:"uppercase", color:"#fff" }}>NEWS <span style={{ color:"#c8ff00" }}>UPDATES</span></div>
              </div>
              <div style={{ flex:1, height:1, background:"linear-gradient(90deg,#2a4018,transparent)" }}/>
              <button style={{ fontFamily:"'Oswald','Barlow Condensed',sans-serif", fontSize:11, fontWeight:600, letterSpacing:".14em", textTransform:"uppercase", padding:"7px 16px", cursor:"pointer", background:"transparent", color:"#c8ff00", border:"1px solid #2a4018" }}
            ...
            <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3,1fr)", gap:12, marginBottom:36 }}>
              {latestNews.map(post => {
                ...
                return (
                  <div key={post.id} onClick={() => setPage("news")}
                    style={{ background:"#0d1209", border:"1px solid #1e2e12", cursor:"pointer", overflow:"hidden" }}>
                    <div style={{ height:2, background:"linear-gradient(90deg," + col + ",transparent)" }}/>
                    {post.image && <div style={{ height:110, overflow:"hidden" }}><img src={post.image} alt="" style={{ width:"100%", height:"100%", objectFit:"cover", opacity:.8 }}/></div>}
                    <div style={{ padding:"12px 14px" }}>
                      <span style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:7, letterSpacing:".12em", color:col, border:"1px solid " + col + "44", padding:"2px 6px" }}>{post.category.toUpperCase()}</span>
                      <div style={{ fontFamily:"'Oswald','Barlow Condensed',sans-serif", fontSize:15, fontWeight:700, color:"#fff", textTransform:"uppercase", letterSpacing:".05em", lineHeight:1.15, margin:"8px 0 5px" }}>{post.title}</div>
                      <div style={{ fontSize:12, color:"#5a6e42", lineHeight:1.6, overflow:"hidden", display:"-webkit-box", WebkitLineClamp:2, WebkitBoxOrient:"vertical" }}>{post.body}</div>
                    </div>
                  </div>
                );
              })}
