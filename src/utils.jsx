...
import { PublicNav as PublicNavNew } from "./utils/nav";
...
:root{
  ...
  --border:#2a2a2a;--text:#e0e0e0;--muted:#888888;--subtle:#444;
...
.pub-nav-link{background:transparent;border:none;color:var(--muted);font-size:11px;font-weight:700;padding:5px 14px;height:auto;cursor:pointer;white-space:nowrap;letter-spacing:.14em;text-transform:uppercase;font-family:'Barlow Condensed',sans-serif;transition:all .15s;position:relative;outline:1px solid transparent;}
.pub-nav-link:hover{color:#fff;outline-color:rgba(255,255,255,.15);background:rgba(255,255,255,.05);}
.pub-nav-link.active{color:var(--accent);outline-color:var(--accent);background:rgba(200,255,0,.07);}
...
.pub-nav-drawer-link{display:flex;align-items:center;gap:14px;padding:14px 20px;font-size:12px;font-weight:700;color:var(--muted);cursor:pointer;border:none;background:none;width:100%;text-align:left;letter-spacing:.14em;text-transform:uppercase;font-family:'Barlow Condensed',sans-serif;transition:all .1s;border-left:2px solid transparent;}
...
.btn{padding:10px 24px;font-size:13px;border-radius:2px;letter-spacing:.15em;font-family:'Barlow Condensed',sans-serif;font-weight:800;text-transform:uppercase;display:inline-block;cursor:pointer;text-decoration:none;text-align:center;line-height:1;box-sizing:border-box;vertical-align:middle;color:inherit;}
...
.btn-ghost{background:transparent;border:1px solid #333;color:var(--text)!important;border-radius:2px;}
...
.modal-box{background:#111;border:1px solid #2a2a2a;padding:28px;width:100%;max-width:520px;max-height:92vh;overflow-y:auto;border-radius:2px;box-shadow:0 24px 80px rgba(0,0,0,.9);}
...
/* ── EVENT CARDS ── */
.event-card{background:var(--bg2);border:1px solid var(--border);overflow:hidden;cursor:pointer;transition:all .15s;position:relative;border-radius:2px;}
...
/* ── SHOP CARDS ── */
.shop-card{background:var(--bg2);border:1px solid var(--border);overflow:hidden;transition:all .15s;border-radius:2px;}
...
.modal-box{width:calc(100vw - 32px);max-width:700px;max-height:90vh;overflow-y:auto;box-sizing:border-box;vertical-align:middle;color:inherit;}
...
.hero-h1{font-family:'Barlow Condensed',sans-serif;font-size:clamp(56px,9vw,110px);line-height:.9;color:#fff;letter-spacing:.02em;margin-bottom:24px;text-transform:uppercase;font-weight:900;text-shadow:0 2px 16px rgba(0,0,0,.85),0 1px 4px rgba(0,0,0,.9);}
.hero-h1 span{color:var(--accent);}
.hero-p{color:#aaa;font-size:15px;line-height:1.7;max-width:520px;margin-bottom:20px;margin-left:auto;margin-right:auto;text-shadow:0 1px 8px rgba(0,0,0,.9);}
...
.hero-stat{flex:1;min-width:50%;max-width:50%;padding:16px 8px;text-align:center;border-right:1px solid #1a2808;box-sizing:border-box;vertical-align:middle;color:inherit;}
...
// ── Public Nav — delegates to utils/nav.jsx (has SiteSearch built in)
function PublicNav(props) {
  return <PublicNavNew {...props} />;
}
...
{(() => {
      ...
      <div className="page-content">
        ...
        {/* UPCOMING EVENTS */}
        {data.events.filter(e => e.published && new Date(e.date + "T" + (e.time || "23:59")) > now).length > 0 && (
          <div style={{ marginBottom:48 }}>
            ...
            <div className="grid-3">
              {data.events.filter(e => e.published && new Date(e.date + "T" + (e.time || "23:59")) > now).sort((a,b) => new Date(a.date) - new Date(b.date)).slice(0, 3).map(ev => {
      ...
      {/* ── LATEST ADS ── */}
      <LatestAdsSection setPage={setPage} cu={cu} />
...
function DesignationInsignia({ desig, size = 56 }) {
  const s = size; const c = "#4fc3f7"; const gold = "#c8a000"; const green = "#c8ff00"; const cx = s / 2; const cy = s / 2;
  const icons = {
    // Achievement
    "SITE OWNER":       <g>
      <polygon points={`${cx},${cy - s*.26} ${cx + s*.1},${cy - s*.08} ${cx + s*.28},${cy - s*.08} ${cx + s*.14},${cy + s*.05} ${cx + s*.2},${cy + s*.25} ${cx},${cy + s*.14} ${cx - s*.2},${cy + s*.25} ${cx - s*.14},${cy + s*.05} ${cx - s*.28},${cy - s*.08} ${cx - s*.1},${cy - s*.08}`} fill="rgba(200,160,0,.2)" stroke={gold} strokeWidth={s*.04}/>
      <circle cx={cx} cy={cy - s*.01} r={s*.09} fill={gold} stroke="none"/>
      <polygon points={`${cx},${cy - s*.26} ${cx + s*.1},${cy - s*.08} ${cx + s*.28},${cy - s*.08} ${cx + s*.14},${cy + s*.05} ${cx + s*.2},${cy + s*.25} ${cx},${cy + s*.14} ${cx - s*.2},${cy + s*.25} ${cx - s*.14},${cy + s*.05} ${cx - s*.28},${cy - s*.08} ${cx - s*.1},${cy - s*.08}`} fill="none" stroke="rgba(255,220,80,.5)" strokeWidth={s*.015}/>
    </g>,
    ...
    "FOUNDING MEMBER":  <g stroke={gold} fill="none" strokeWidth={s*.033}><circle cx={cx} cy={cy} r={s*.22}/><circle cx={cx} cy={cy} r={s*.13}/><line x1={cx} y1={cy - s*.22} x2={cx} y2={cy - s*.13}/><line x1={cx} y1={cy + s*.13} x2={cx} y2={cy + s*.22}/><line x1={cx - s*.22} y1={cy} x2={cx - s*.13} y2={cy}/><line x1={cx + s*.13} y1={cy} x2={cx + s*.22} y2={cy}/><circle cx={cx} cy={cy} r={s*.04} fill={gold}/></g>,
    "CENTURION":        <g stroke={gold} fill="none" strokeWidth={s*.033}><polygon points={`${cx},${cy - s*.24} ${cx + s*.05},${cy - s*.08} ${cx + s*.23},${cy - s*.08} ${cx + s*.09},${cy + s*.03} ${cx + s*.14},${cy + s*.22} ${cx},${cy + s*.12} ${cx - s*.14},${cy + s*.22} ${cx - s*.09},${cy + s*.03} ${cx - s*.23},${cy - s*.08} ${cx - s*.05},${cy - s*.08}`} fill="rgba(200,160,0,.1)"/><text x={cx} y={cy + s*.08} textAnchor="middle" fontSize={s*.18} fill={gold} stroke="none" fontWeight="700">C</text></g>,
    // Role
    "MARSHAL":          <g stroke={green} fill="none" strokeWidth={s*.033}><rect x={cx - s*.18} y={cy - s*.2} width={s*.36} height={s*.4} rx={s*.03}/><line x1={cx - s*.18} y1={cy - s*.05} x2={cx + s*.18} y2={cy - s*.05}/><circle cx={cx} cy={cy + s*.1} r={s*.06} fill={green}/></g>,
    "REFEREE":          <g stroke={c} fill="none" strokeWidth={s*.033}><circle cx={cx} cy={cy} r={s*.2}/><line x1={cx - s*.14} y1={cy - s*.14} x2={cx + s*.14} y2={cy + s*.14}/><line x1={cx - s*.07} y1={cy - s*.2} x2={cx - s*.07} y2={cy + s*.2}/></g>,
    "PHOTOGRAPHER":     <g stroke={c} fill="none" strokeWidth={s*.033}><rect x={cx - s*.2} y={cy - s*.12} width={s*.4} height={s*.28} rx={s*.04}/><circle cx={cx} cy={cy + s*.02} r={s*.09}/><rect x={cx - s*.06} y={cy - s*.2} width={s*.12} height={s*.08} rx={s*.02}/><circle cx={cx - s*.13} cy={cy - s*.06} r={s*.025} fill={c}/></g>,
    "CONTENT CREATOR":  <g stroke={c} fill="none" strokeWidth={s*.033}><polygon points={`${cx - s*.18},${cy - s*.18} ${cx + s*.22},${cy} ${cx - s*.18},${cy + s*.18}`} fill="rgba(79,195,247,.12)"/><circle cx={cx} cy={cy} r={s*.24}/></g>,
    // Honour
    "MOST VALUABLE PLAYER": <g strokeWidth={s*.033}><polygon points={`${cx},${cy - s*.22} ${cx + s*.07},${cy - s*.07} ${cx + s*.23},${cy - s*.07} ${cx + s*.11},${cy + s*.04} ${cx + s*.16},${cy + s*.22} ${cx},${cy + s*.13} ${cx - s*.16},${cy + s*.22} ${cx - s*.11},${cy + s*.04} ${cx - s*.23},${cy - s*.07} ${cx - s*.07},${cy - s*.07}`} fill="rgba(200,160,0,.15)" stroke={gold}/><text x={cx} y={cy + s*.08} textAnchor="middle" fontSize={s*.15} fill={gold} stroke="none" fontWeight="700">MVP</text></g>,
    "FAIR PLAY":        <g stroke={green} fill="none" strokeWidth={s*.036}><path d={`M${cx - s*.16},${cy - s*.06} L${cx - s*.04},${cy + s*.1} L${cx + s*.18},${cy - s*.14}`} strokeLinecap="round" strokeLinejoin="round"/><circle cx={cx} cy={cy} r={s*.24}/></g>,
    "IRON MAN":         <g stroke={c} fill="none" strokeWidth={s*.033}><rect x={cx - s*.14} y={cy - s*.22} width={s*.28} height={s*.44} rx={s*.06}/><line x1={cx - s*.14} y1={cy - s*.06} x2={cx + s*.14} y2={cy - s*.06}/><line x1={cx - s*.14} y1={cy + s*.06} x2={cx + s*.14} y2={cy + s*.06}/><circle cx={cx} cy={cy - s*.13} r={s*.04} fill={c}/></g>,
    "SNIPER ELITE":     <g stroke={c} fill="none" strokeWidth={s*.033}><circle cx={cx} cy={cy} r={s*.18}/><line x1={cx} y1={cy - s*.28} x2={cx} y2={cy - s*.18}/><line x1={cx} y1={cy + s*.18} x2={cx} y2={cy + s*.28}/><line x1={cx - s*.28} y1={cy} x2={cx - s*.18} y2={cy}/><line x1={cx + s*.18} y1={cy} x2={cx + s*.28} y2={cy}/><circle cx={cx} cy={cy} r={s*.04} fill={c}/><circle cx={cx} cy={cy} r={s*.11} strokeDasharray={`${s*.05} ${s*.05}`}/></g>,
    // Fun
    "WALK-ON KING":     <g stroke={c} fill="none" strokeWidth={s*.033}><circle cx={cx - s*.08} cy={cy} r={s*.1}/><circle cx={cx + s*.1} cy={cy - s*.04} r={s*.1}/><polyline points={`${cx - s*.18},${cy + s*.18} ${cx - s*.08},${cy + s*.1} ${cx + s*.1},${cy + s*.06} ${cx + s*.22},${cy + s*.14}`} strokeLinecap="round"/></g>,
    "LAST STAND":       <g stroke={c} fill="none" strokeWidth={s*.033}><polygon points={`${cx},${cy - s*.24} ${cx + s*.22},${cy + s*.14} ${cx - s*.22},${cy + s*.14}`} fill="rgba(79,195,247,.06)"/><line x1={cx} y1={cy - s*.12} x2={cx} y2={cy + s*.04}/><circle cx={cx} cy={cy + s*.1} r={s*.03} fill={c}/></g>,
    "RESPAWN":          <g stroke={c} fill="none" strokeWidth={s*.033}><path d={`M${cx + s*.18},${cy} A${s*.18},${s*.18} 0 1 0 ${cx},${cy - s*.18}`} strokeLinecap="round"/><polyline points={`${cx - s*.04},${cy - s*.24} ${cx},${cy - s*.18} ${cx + s*.08},${cy - s*.22}`} strokeLinecap="round" strokeLinejoin="round"/></g>,
  };
  return (
    <svg width={s} height={s} viewBox={`0 0 ${s} ${s}`} style={{ display: "block" }}>
      <rect width={s} height={s} fill="#080a06" rx={s * .04}/>
