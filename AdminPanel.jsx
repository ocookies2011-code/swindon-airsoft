// AdminPanel.jsx — Shell/router. All section components are imported from their own files.
...
import AdminUkaraApplications from "./AdminUkara";
...
import { AdminFailedPayments } from "./AdminStats";
import AdminLeaderboard       from "./AdminLeaderboard";
import AdminVisitorStats      from "./AdminStats";
import AdminRevenue           from "./AdminRevenue";
import AdminQA, { AdminGallery } from "./AdminContent";
import AdminContactDepts      from "./AdminContact";
...
// SA_LOGO_SRC lives in its own file to keep this file readable
import SA_LOGO_SRC            from "./saLogo";

function AdminPanel({ data, cu, save, updateUser, updateEvent, showToast, setPage, refresh }) {
  const getInitialSection = () => {
    const parts = window.location.hash.replace("#", "").split("/");
    const ADMIN_SECTIONS = [
      "dashboard", "events", "waivers", "unsigned-waivers", "players", "shop",
      "leaderboard-admin", "revenue", "visitor-stats", "gallery-admin", "qa-admin",
      "staff-admin", "contact-admin", "messages", "cash", "purchase-orders",
      "discount-codes", "gift-vouchers", "settings", "audit-log", "cheat-reports", "ukara-admin",
    ];
  ...
  const setSection = (s) => { setSectionState(s); window.location.hash = "admin/" + s; };
  ...
  useEffect(() => {
    const fetchReports = () => {
      supabase.from("cheat_reports").select("id", { count: "exact", head: true }).eq("status", "pending")
        .then(({ count }) => setPendingReports(count || 0)).catch(() => {});
    };
    fetchReports();
  ...
  const [pendingUkara, setPendingUkara] = useState(0);
  useEffect(() => {
        ...
        <div style={{ padding: "8px 8px 0" }}>
          ...
          <div style={{ marginTop: 16, padding: "0 0 16px" }}>
            ...
            <div className="sb-item" onClick={() => setPage("home")}>
              <span className="sb-icon"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#ef9a9a" strokeWidth="2"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg></span>
      ...
      <div className="admin-main">
        ...
        <div className="admin-content">
          ...
          <div style={{ display: section === "events" ? "block" : "none" }}>
            <AdminEventsBookings data={data} save={save} updateEvent={updateEvent} updateUser={updateUser} showToast={showToast} cu={cu} />
          </div>
          ...
          <div style={{ display: section === "players" ? "block" : "none" }}>
            <AdminPlayers data={data} save={save} updateUser={updateUser} showToast={showToast} cu={cu} />
          </div>
          {section === "cheat-reports"    && <AdminCheatReports data={data} showToast={showToast} cu={cu} />}
          <div style={{ display: section === "shop" ? "block" : "none" }}>
            <AdminShop data={data} save={save} showToast={showToast} cu={cu} />
          </div>
